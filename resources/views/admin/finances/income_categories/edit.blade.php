...
@section('title', '| Edit Kategori Pemasukan: ' . $incomeCategory->name)
                    ...
                    <form action="{{ route('admin.income-categories.update', $incomeCategory->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="description" rows="5"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">{{ old('description', $incomeCategory->description) }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
